...
class _UsersOverContentState extends State<UsersOverContent> {
  ...
  Future<SubscriptionAnalytics> _loadAnalytics() {
    ...
    return provider.getNewUsersSubscriptionAnalytics(
      year: now.year,
      month: _yearMode ? null : now.month,
    );
  }
  ...
  Future<List<int>> _buildUsersPdfBytes({
    ...
    final subscriptionIntervalText = _yearMode
        ? "Year ${now.year}"
        : "Month ${DateFormat('MMMM').format(DateTime(now.year, now.month))} ${now.year}";
                            ...
                            builder: (context) {
                              ...
                              if (growthSnapshot.hasError) {
                                return Container(
                                  ...
                                  child: Text(
                                    "Error while loading user growth: ${growthSnapshot.error}",
                                    style: const TextStyle(
                                      color: Colors.red,
                                      fontSize: 14,
                                    ),
